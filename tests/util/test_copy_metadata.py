...
def test_copy_metadata_props():
    ...
    src_node = onnx.NodeProto(metadata_props=[add_metadata("key1", "value1"), add_metadata("key2", "value2")])
...
def test_copy_metadata_props_existing_target_md(mode):
    ...
    dst_node = onnx.NodeProto(metadata_props=[add_metadata("key1", "value3")])
...
def test_copy_metadata_props_bad_mode():
    src_node = onnx.NodeProto(metadata_props=[add_metadata("key1", "value1")])
    dst_node = onnx.NodeProto()
...
def test_copy_metadata_props_gemm2matmul():
    ...
    gemm_node.metadata_props.extend([add_metadata("key1", "value1"), add_metadata("key2", "value2")])
...
def test_copy_metadata_props_batchnorm2affine():
    ...
    bn_node.metadata_props.extend([add_metadata("key1", "value1"), add_metadata("key2", "value2")])
